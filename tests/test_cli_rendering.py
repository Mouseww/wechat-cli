...
from wechat_cli.cli import messages, send, sessions
...
class CliRenderingTest(unittest.TestCase):
    ...
    def test_messages_uses_lightweight_weflow_request(self):
        runner = CliRunner()
        calls = []

        def fake_weflow_get(path, params, config):
            calls.append((path, params, config))
            return {
                "success": True,
                "count": 1,
                "messages": [
        ...
        class UnexpectedWeFlowClient:
            def __init__(self, *_args, **_kwargs):
                raise AssertionError("messages should not initialize WeFlowClient")

        with (
            patch("wechat_cli.cli._load_weflow_config", return_value={"weflow_url": "http://127.0.0.1:5031"}),
            patch("wechat_cli.cli._weflow_get", side_effect=fake_weflow_get),
            patch("wechat_cli.cli._get_weflow_client_class", return_value=UnexpectedWeFlowClient),
        ):
            result = runner.invoke(messages, ["wxid_test", "--limit", "1"])
        ...
        self.assertEqual(
            ...
            [
                (
                    "/api/v1/messages",
                    {"talker": "wxid_test", "limit": 1, "offset": 0},
                    {"weflow_url": "http://127.0.0.1:5031"},
                )
            ],
        )
